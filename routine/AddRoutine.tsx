...
import { addRoutine } from '@/storage/routineStorage';
...
export const AddRoutine = forwardRef<BottomSheetModal, { onAdded: () => void }>(
  ({ onAdded }, ref) => {
    ...
    const addFeedingTime = () => feedingTimes.length < 4 && setFeedingTimes([...feedingTimes, '12:00']);
    const removeFeedingTime = (index: number) => setFeedingTimes(feedingTimes.filter((_, i) => i !== index));
    ...
    const handleAdd = async () => {
        ...
        //     break;
        //     case 'trash':
        //     config = { pickupDays: selectedDays, isRecycling };
        //     break;
        // }
        ...
        try {
            ...
            text2: `${name || routineTypeConfig[type].label} added successfully`,
            });

            onAdded(); // close sheet + refresh list
        } catch (err) {
            ...
            text2: 'Could not create routine. Please try again.',
            });
        } finally {
            setIsSaving(false);
        }
      ...
      >
        ...
        >
          ...
          <View className="flex-row justify-between items-center mb-5">
            <Text className="text-2xl font-bold text-foreground">Add Routine</Text>
            <Pressable onPress={() => onAdded()} className='rounded-xl h-9 w-9 items-center justify-center bg-secondary'>
          ...
          <View className="mb-5">
            <Text className="text-lg font-bold text-foreground mb-3">Routine Type</Text>
            <Pressable 
              onPress={() => setIsDropdownOpen(!isDropdownOpen)}
              className="flex-row items-center justify-between bg-secondary p-3 rounded-2xl border border-secondary shadow-sm"
            >
                <View className="flex-row items-center justify-between p-2 gap-1">
                    <Text className="text-2xl">{routineTypeConfig[type].emoji}</Text>
                    <Text className="text-base font-medium text-foreground ml-3">{routineTypeConfig[type].label}</Text>
                </View>
                <Feather name={isDropdownOpen ? "chevron-up" : "chevron-down"} size={20} color="#2F3A36" />
            </Pressable>

            {isDropdownOpen && (
              <View className="mt-2 bg-card rounded-2xl border border-card shadow-xl overflow-hidden">
                {(Object.keys(routineTypeConfig) as RoutineType[]).map((t) => (
                  <Pressable 
                    key={t} 
                    onPress={() => handleTypeSelect(t)}
                    className={`flex-row items-center justify-between p-4 ${type === t ? 'bg-secondary' : ''}`}
                  >
                    <View className="flex-row items-center justify-between p-1 gap-1">
                        <Text className="text-2xl">{routineTypeConfig[t].emoji}</Text>
                        <Text className="text-base font-medium text-foreground ml-3">{routineTypeConfig[t].label}</Text>
                    </View>

                    {type === t && <Feather name="check" size={20} color="#6B9E9E" />}
                  </Pressable>
                ))}
              </View>
            )}
          ...
          {/* Name Input */}
           <View className="mb-5">
                <Text className="text-lg font-bold text-foreground mb-3">Name</Text>
                <TextInput
          ...
          <View className="mb-11">
            {type === 'laundry' && (
                <View>
                    <View className="flex-row justify-between mb-4">
                        <Text className="text-lg font-bold text-foreground mb-3">Duration</Text>
                        <Text className="text-primary font-bold mr-2">{durationMinutes}m</Text>
                    </View>
                    <Slider
                    value={durationMinutes}
                    onValueChange={setDurationMinutes}
                    minimumValue={15} maximumValue={120} step={5}
                    minimumTrackTintColor="#6B9E9E" thumbTintColor="#6B9E9E"
                    />
                </View>
            )}
            ...
            {type === 'pet' && (
              <View>
                <Text className="text-lg font-bold text-foreground mb-3">Feeding Times</Text>
                {feedingTimes.map((time, i) => (
                    <View key={i} className='flex-row items-center gap-2 mb-2'>
                        ...
                        >
                            <Text className="text-base font-medium">{formatTime12h(time)}</Text>
                            <Feather name="clock" size={18} color="#6B9E9E" />
                        </Pressable>
                        <Pressable onPress={() => removeFeedingTime(i)} className="p-3 bg-red-50 rounded-xl"> 
                ...
                <Pressable onPress={addFeedingTime} className="mt-2">
                  <Text className="text-primary font-semibold text-sm">+ Add feeding time</Text>
                </Pressable>
              </View>
            )}
            ...
            {type === 'trash' && (
              <View>
                <Text className="text-lg font-bold text-foreground mb-3">Pickup Days</Text>
                <View className="flex-row flex-wrap gap-2">
                  {weekDays.map((day, i) => (
                    ...
                    >
                      <Text className={`text-xs font-bold ${selectedDays.includes(i) ? 'text-white' : 'text-gray-500'}`}>{day}</Text>
                    </Pressable>
                  ))}
                </View>
              </View>
            ...
            <View>
                <View className="flex-row justify-between mb-4">
                  <Text className="text-lg font-bold text-foreground mb-3">Watering Interval (days)</Text>
...
{/* 
          ...
          {/* <Pressable
            onPress={handleAdd}
            className="bg-[#6B9E9E] rounded-xl py-4 px-5 items-center mb-8"
          >
            <Text className="text-white font-bold text-base">Add Routine</Text>
          </Pressable> */}
          <Pressable
            disabled={isSaving}
            onPress={handleAdd}
            className={`rounded-xl py-4 px-5 items-center mb-8 ${
